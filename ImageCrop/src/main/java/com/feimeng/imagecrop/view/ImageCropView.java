...
package com.feimeng.imagecrop.view;
...
import com.feimeng.imagecrop.R;
import com.feimeng.imagecrop.model.CropInfo;
import com.feimeng.imagecrop.model.ViewState;
import com.feimeng.imagecrop.util.BitmapLoadUtils;
import com.feimeng.imagecrop.view.graphics.FastBitmapDrawable;
...
public class ImageCropView extends androidx.appcompat.widget.AppCompatImageView {
    ...
    public static final int OUTLINE_GRID = 0; // 方格
    public static final int OUTLINE_CIRCLE = 1; // 圆
    ...
    private Bitmap mOutsideLayerCircleBitmap;
    ...
    private int mOutlineStyle;
    private int mGridInnerMode;
    private int mGridOuterMode;
    private float mGridLeftRightMargin;
    private float mGridTopBottomMargin;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mOutlineStyle = a.getInt(R.styleable.ImageCropView_outlineStyle, OUTLINE_GRID);
        ...
        mGridInnerMode = a.getInt(R.styleable.ImageCropView_setInnerGridMode, GRID_OFF);
        mGridOuterMode = a.getInt(R.styleable.ImageCropView_setOuterGridMode, GRID_OFF);

        mGridLeftRightMargin = a.getDimension(R.styleable.ImageCropView_gridLeftRightMargin, 0);
        mGridTopBottomMargin = a.getDimension(R.styleable.ImageCropView_gridTopBottomMargin, 0);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (height > mThisHeight) {
            int width = (int) ((mThisHeight - (mGridTopBottomMargin * 2)) / mTargetAspectRatio);
            int halfDiff = (mThisWidth - width) / 2;
            mCropRect.set(left + halfDiff, top + mGridTopBottomMargin, right - halfDiff, bottom - mGridTopBottomMargin);
        } else {
            height = (int) ((mThisWidth - (mGridLeftRightMargin * 2)) * mTargetAspectRatio);
            int halfDiff = (mThisHeight - height) / 2;
            mCropRect.set(left + mGridLeftRightMargin, halfDiff - top, right - mGridLeftRightMargin, height + halfDiff);
        }
    ...
    private void drawTransparentLayer(Canvas canvas) {
        ...
        canvas.drawRect(mVisibleRect.left, mVisibleRect.top, mVisibleRect.right, mCropRect.top, mOutsideLayerPaint);                          // top
        canvas.drawRect(mVisibleRect.left, mCropRect.bottom, mVisibleRect.right, mVisibleRect.bottom, mOutsideLayerPaint);                    // bottom
        canvas.drawRect(mVisibleRect.left, mCropRect.top, mCropRect.left, mCropRect.bottom, mOutsideLayerPaint);        // left
        canvas.drawRect(mCropRect.right, mCropRect.top, mVisibleRect.right, mCropRect.bottom, mOutsideLayerPaint);      // right

        if (mOutlineStyle == OUTLINE_CIRCLE) { // 圆
            if (mOutsideLayerCircleBitmap == null) {
                Paint paint = new Paint();
                paint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.SRC_OUT));
                mOutsideLayerCircleBitmap = Bitmap.createBitmap((int) mCropRect.width(), (int) mCropRect.height(), Bitmap.Config.ARGB_8888);
                Canvas circleCanvas = new Canvas(mOutsideLayerCircleBitmap);
                circleCanvas.drawRect(0, 0, mCropRect.width(), mCropRect.height(), paint);      // right
                float halfLineWidth = mGridOuterLinePaint.getStrokeWidth() * 0.5f;
                circleCanvas.drawCircle(mCropRect.width() / 2F, mCropRect.height() / 2F, mCropRect.width() / 2 - halfLineWidth, paint);
            }
            float left = (mVisibleRect.width() - mCropRect.width()) / 2;
            float top = (mVisibleRect.height() - mCropRect.height()) / 2;
            canvas.drawBitmap(mOutsideLayerCircleBitmap, left, top, mOutsideLayerPaint);
        }
    }
    ...
    private void drawGrid(Canvas canvas) {
        ...
        if (mOutlineStyle == OUTLINE_GRID) { // 方格
            if (mGridInnerMode == GRID_ON) {
                canvas.drawLines(mPts, mGridInnerLinePaint);
            }
            if (mGridOuterMode == GRID_ON) {
                float halfLineWidth = mGridOuterLinePaint.getStrokeWidth() * 0.5f;
            ...
            if (mGridOuterMode == GRID_ON) {
                float halfLineWidth = mGridOuterLinePaint.getStrokeWidth() * 0.5f;
                canvas.drawCircle(mVisibleRect.width() / 2F, mVisibleRect.height() / 2F, mCropRect.width() / 2 - halfLineWidth, mGridOuterLinePaint);
            }
        }
    }
    ...
    public Bitmap getCroppedImage() {
        ...
        if (mImageFilePath != null) {
            bitmap = cropInfo.getCroppedImage(mImageFilePath);
        } else {
    ...
    public void setGridInnerMode(int gridInnerMode) {
        this.mGridInnerMode = gridInnerMode;
        invalidate();
    }

    public void setGridOuterMode(int gridOuterMode) {
        this.mGridOuterMode = gridOuterMode;
        invalidate();
    }

    public void setGridLeftRightMargin(int marginDP) {
        this.mGridLeftRightMargin = dpToPixel(marginDP);
        requestLayout();
    }

    public void setGridTopBottomMargin(int marginDP) {
        this.mGridTopBottomMargin = dpToPixel(marginDP);
        requestLayout();
    }
