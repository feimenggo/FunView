 ...
 * 水平层叠布局
 */
public class StackLineLayout extends ViewGroup {
    /**
     * 两个View之间距的比例 默认是在一半的位置
     */
    private float mViewMarginRate;
    /**
     * 是不是从后面向前摆放 默认第一个在上面
     */
    private boolean mIsReverse = true;

    public StackLineLayout(Context context) {
        this(context, null);
    }

    public StackLineLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public StackLineLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray array = context.obtainStyledAttributes(attrs, R.styleable.StackLineLayout);
        // 默认是在一半的位置
        mViewMarginRate = array.getFloat(R.styleable.StackLineLayout_lineViewMarginRate, 0.5f);
        // 默认第一个在上面
        mIsReverse = array.getBoolean(R.styleable.StackLineLayout_lineIsReverse, mIsReverse);
        array.recycle();
        // 设置允许改变绘制顺序
