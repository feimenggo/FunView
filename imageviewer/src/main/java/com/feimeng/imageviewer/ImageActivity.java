        ...
        try {
            for (int i = 0; i < contentViewOriginModels.size(); i++) {
                ImageFragment imageFragment = ImageFragment.newInstance(
                        imageUrls[i], i, mViewerConfig.getType(),
                        contentViewOriginModels.size() == 1 || mViewerConfig.getPosition() == i, contentViewOriginModels.get(i)
                );
                fragmentList.add(imageFragment);
            }
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(this, "出错啦，图片数目不一致", Toast.LENGTH_LONG).show();
        }
