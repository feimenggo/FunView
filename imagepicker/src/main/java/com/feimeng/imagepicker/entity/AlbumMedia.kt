...
class AlbumMedia : Parcelable {
    ...
    override fun equals(obj: Any?): Boolean {
        if (obj !is AlbumMedia) {
            return false
        }

        val other = obj as AlbumMedia?
        return (id == other!!.id
    ...
    companion object {
        @JvmField
        val CREATOR: Parcelable.Creator<AlbumMedia> = object : Parcelable.Creator<AlbumMedia> {
            override fun createFromParcel(source: Parcel): AlbumMedia? {
                return AlbumMedia(source)
            }

            override fun newArray(size: Int): Array<AlbumMedia?> {
        ...
        fun valueOf(cursor: Cursor): AlbumMedia {
            return AlbumMedia(cursor.getLong(cursor.getColumnIndex(MediaStore.Files.FileColumns._ID)),
